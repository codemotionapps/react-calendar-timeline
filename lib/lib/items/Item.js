...
var Item = function (_Component) {
  ...
  _createClass(Item, [{
    ...
    value: function mountInteract() {
      ...
      (0, _interactjs2.default)(this.item).resizable({
      ...
      }).styleCursor(false).on('resizestart', function (e) {
        ...
        if (_this2.state.resizing) {
          ...
          if (_this2.props.onResized) {
            _this2.props.onResized(_this2.itemId, resizeTime, resizeEdge, _this2.resizeTimeDelta(e, resizeEdge), _this2.props.item.type);
          }
          _this2.setState({
            resizing: null,
            resizeStart: null,
    ...
    value: function componentDidUpdate(prevProps) {
      ...
      if (this.item) {
        (0, _interactjs2.default)(this.item).dropzone({
          accept: '.dragged-point',
          overlap: 0.75,
          ondragenter: function ondragenter(event) {
            _this3.props.onPointEnter(event, _this3.itemId, event.relatedTarget.id);
          },
          ondrop: function ondrop(event) {
            _this3.props.onPointDrop(event, _this3.itemId, event.relatedTarget.id);
          },
          ondragleave: function ondragleave(event) {
            _this3.props.onPointLeave(event, _this3.itemId, event.relatedTarget.id);
          }
        });
      }
    }
    ...
    value: function componentWillUnmount() {
      this.item && (0, _interactjs2.default)(this.item).unset();
    }
  }, {
    key: 'actualClick',
...
var _initialiseProps = function _initialiseProps() {
  ...
  this.onMouseDown = function (e) {
    if (!_this4.state.interactMounted) {
      e.preventDefault();
      _this4.startedClicking = true;
    }
  };

  this.onMouseUp = function (e) {
    if (!_this4.state.interactMounted && _this4.startedClicking) {
      _this4.startedClicking = false;
      _this4.actualClick(e, 'click');
    }
  };
  ...
  this.onTouchStart = function (e) {
    if (!_this4.state.interactMounted) {
      e.preventDefault();
      _this4.startedTouching = true;
    }
  };

  this.onTouchEnd = function (e) {
    if (!_this4.state.interactMounted && _this4.startedTouching) {
      _this4.startedTouching = false;
      _this4.actualClick(e, 'touch');
    }
  };
  ...
  this.handleDoubleClick = function (e) {
    ...
    if (_this4.props.onItemDoubleClick) {
      _this4.props.onItemDoubleClick(_this4.itemId, e);
    }
  };

  this.handleContextMenu = function (e) {
    if (_this4.props.onContextMenu) {
      e.preventDefault();
      e.stopPropagation();
      _this4.props.onContextMenu(_this4.itemId, e);
    }
  };

  this.getItemRef = function (el) {
    return _this4.item = el;
  };

  this.getDragLeftRef = function (el) {
    return _this4.dragLeft = el;
  };

  this.getDragRightRef = function (el) {
    return _this4.dragRight = el;
  };
  ...
  this.getItemProps = function () {
    ...
    var classNames = 'rct-item' + (_this4.props.item.className ? ' ' + _this4.props.item.className : '');

    return {
      key: _this4.itemId,
      ref: _this4.getItemRef,
      title: _this4.itemDivTitle,
      className: classNames + (' ' + (props.className ? props.className : '')),
      onMouseDown: (0, _events.composeEvents)(_this4.onMouseDown, props.onMouseDown),
      onMouseUp: (0, _events.composeEvents)(_this4.onMouseUp, props.onMouseUp),
      onTouchStart: (0, _events.composeEvents)(_this4.onTouchStart, props.onTouchStart),
      onTouchEnd: (0, _events.composeEvents)(_this4.onTouchEnd, props.onTouchEnd),
      onDoubleClick: (0, _events.composeEvents)(_this4.handleDoubleClick, props.onDoubleClick),
      onContextMenu: (0, _events.composeEvents)(_this4.handleContextMenu, props.onContextMenu),
      style: Object.assign({}, _this4.getItemStyle(props))
    };
  };
